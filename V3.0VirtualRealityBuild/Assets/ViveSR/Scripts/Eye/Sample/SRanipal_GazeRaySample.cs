        ...
        namespace Eye
        {
            public class SRanipal_GazeRaySample : MonoBehaviour
            {
                ...
                private void Update()
                {
                    ...
                    //line renderer (muted when visual is not needed)
                    GazeRayRenderer.SetPosition(0, Camera.main.transform.position - Camera.main.transform.up * 0.05f);
                    //GazeRayRenderer.SetPosition(0, Camera.main.transform.position + GazeDirectionCombined * (LengthOfRay-1));
                    GazeRayRenderer.SetPosition(1, Camera.main.transform.position + GazeDirectionCombined * LengthOfRay);
                    GAZE = GazeDirectionCombined.ToString();

                    //raycast
                    if(Physics.Raycast(Camera.main.transform.position, GazeDirectionCombined, out RaycastHit hitinfo, LengthOfRay))
                    {
                        GAZECATEGORY = hitinfo.collider.ToString();
                        Debug.Log(GAZECATEGORY);
                    }
                }
